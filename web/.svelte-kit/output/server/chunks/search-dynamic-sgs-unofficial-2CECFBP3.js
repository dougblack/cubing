...
import { n as puzzles } from "./chunk-FLK6AZKB.js";
import { n as randomUIntBelow } from "./esm.js";
import { t as parseSGS } from "./chunk-ROB5TROI.js";
//#region ../core/node_modules/cubing/dist/lib/cubing/chunks/search-dynamic-sgs-unofficial-2CECFBP3.js
...
function index_to_permutation(ind, n) {
	...
	for (let i = n - 2; i >= 0; i--) for (let j = i + 1; j < n; j++) perm[j] += +(perm[j] >= perm[i]);
	return perm;
}
function permutation_parity(A) {
	let n = A.length;
	let parity = 0;
	for (let i = 0; i < n - 1; i++) for (let j = i; j < n; j++) if (A[i] > A[j]) parity ^= 1;
	return parity;
}
function index_to_evenpermutation(ind, n) {
	...
	perm[n - 1] = 0;
	for (let i = n - 2; i >= 0; i--) for (let j = i + 1; j < n; j++) if (perm[j] >= perm[i]) perm[j]++;
	else parity ^= 1;
...
function permutation_from_cycle(cycle, n) {
	...
	for (let i = 0; i < cycle.length; i++) perm[cycle[i]] = cycle[(i + 1) % cycle.length];
	return perm;
}
...
var move_UL = [permutation_from_cycle([
	0,
	1,
	4
], 12), unsparsify_list({ 0: 2 }, 8)];
var move_U = [permutation_from_cycle([
	1,
	2,
	5
], 12), unsparsify_list({ 1: 2 }, 8)];
var move_UR = [permutation_from_cycle([
	2,
	3,
	6
], 12), unsparsify_list({ 2: 2 }, 8)];
var move_F = [permutation_from_cycle([
	3,
	0,
	7
], 12), unsparsify_list({ 3: 2 }, 8)];
var move_L = [permutation_from_cycle([
	9,
	8,
	4
], 12), unsparsify_list({ 4: 2 }, 8)];
var move_B = [permutation_from_cycle([
	10,
	9,
	5
], 12), unsparsify_list({ 5: 2 }, 8)];
var move_R = [permutation_from_cycle([
	11,
	10,
	6
], 12), unsparsify_list({ 6: 2 }, 8)];
var move_D = [permutation_from_cycle([
	8,
	11,
	7
], 12), unsparsify_list({ 7: 2 }, 8)];
index_to_permutation(0, 12), Array(8).fill(0);
var moves = [
	move_UL,
	move_U,
	move_UR,
...
function stringify_move_sequence(move_sequence) {
	...
	return move_sequence.map(([m, r]) => move_names[m] + suffixes[r]).join(" ");
}
function generate_random_state() {
	let p = index_to_evenpermutation(Math.floor(randomUIntBelow(factorial(12)) / 2), 12);
	let o = Array(8);
...
function solve(state) {
	...
	let phase1_mtables = [generate_phase1_edge_mtable(), generate_phase1_separate_mtable()];
	let phase1_ptables = [generate_phase1_edge_ptable(), generate_phase1_separate_ptable()];
	let phase2_mtables = [generate_phase2_edge_mtable(), generate_phase2_corner_mtable()];
	let phase2_ptables = [generate_phase2_edge_ptable(), generate_phase1_corner_ptable()];
	...
	for (let i = 0; i < 25; i++) {
		...
		let corner_ind = 0;
		for (let i2 = 0; i2 < 4; i2++) corner_ind += new_state[1][i2] * 3 ** i2;
		let sol2 = ida_solve([edge_ind, corner_ind], phase2_mtables, phase2_ptables, best ? best.length - sol1.length - 1 : 999999);
		if (sol2 === void 0) continue;
		if (best === void 0 || best.length > sol1.length + sol2.length) best = sol1.concat(sol2);
		if (/* @__PURE__ */ new Date() - start_time > 300) break;
	}
	return best;
}
...
function generate_phase1_edge_mtable() {
	...
	for (let i = 0; i < 12 ** 4; i++) {
		...
		for (let m = 0; m < 8; m++) {
			let I = 0;
			for (let j = 0; j < 4; j++) I += mtable_single[Math.floor(i / 12 ** j) % 12][m] * 12 ** j;
			mtable[i][m] = I;
		}
	}
...
function generate_phase1_edge_ptable() {
	...
	return tables.phase1ep = bfs(generate_phase1_edge_mtable(), [20564]);
}
function generate_phase1_separate_mtable() {
	...
	let mtable = [];
	for (let j = 0; j < C12_4; j++) for (let i = 0; i < 81; i++) {
	...
	for (let i = 0; i < HALFFACT8; i++) {
		...
		do
			for (let k = n - 2; k >= 0; k--) {
				...
				parity ^= 1;
				for (let j = 0; k + 1 + j < n - 1 - j; j++, parity ^= 1) [perm[k + 1 + j], perm[n - 1 - j]] = [perm[n - 1 - j], perm[k + 1 + j]];
				break;
			}
		while (parity !== 0);
	}
...
function ida_search(indices, mtables, ptables, bound, last) {
	...
	for (let m = 0; m < nmoves; m++) {
		...
		while (indices.some((_, i) => indices[i] !== new_indices[i])) {
			...
			if (subpath !== void 0) return [[m, r]].concat(subpath);
...
//#endregion
export { getRandomRediCubeScramble, sgsDataFTO };
