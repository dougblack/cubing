...
//#region ../core/node_modules/cubing/dist/lib/cubing/chunks/search-dynamic-solve-master_tetraminx-GIS7T5B7.js
...
var [evenpermutation12_to_index, index_to_evenpermutation12] = (() => {
	...
	for (let i = 0; i < 4096; i++) for (let j = 0, counter = 0; j < 12; j++) {
		if ((i >>> j & 1) === 0) continue;
...
function compose_state(state1, state2) {
	let co = Array(4);
	for (let i = 0; i < 4; i++) co[i] = (state1.co[i] + state2.co[i]) % 3;
	return {
		co,
		mp: compose(state1.mp, state2.mp),
		wp: compose(state1.wp, state2.wp),
		cp: compose(state1.cp, state2.cp)
	};
}
identity_permutation(12), identity_permutation(12);
var move_U = {
	co: [
		2,
		0,
...
function generate_random_state() {
	let co = Array(4);
	for (let i = 0; i < 4; i++) co[i] = randomUIntBelow(3);
	...
	return {
		co,
		mp,
		wp: index_to_evenpermutation(randomUIntBelow(factorial(12) / 2), 12),
		cp: index_to_evenpermutation(randomUIntBelow(factorial(4) / 2), 4)
	};
}
...
function generate_scramble_sequence(tips = true, obfuscate_tips = false) {
	...
	if (!obfuscate_tips) {
		for (let i = 0; i < 4; i++) {
			...
			if (x !== 0) scramble_string += ` ${tip_names[i]}${suffixes[x]}`;
		}
	...
	while (!(weight(amount_pre) >= 1 && weight(amount_post) >= 1 && weight(amount_pre) + weight(amount_post) >= 4)) for (let i = 0; i < 4; i++) {
		amount_pre[i] = randomUIntBelow(3);
		amount_post[i] = (amount[i] - amount_pre[i] + 3) % 3;
	}
...
function solve(state) {
	...
	for (let i = 0; i < 22; i++) {
		...
		let sol2 = ida_solve_gen(index_phase2(new_state), phase2_mtables, phase2_ptables, best ? best.length - sol1.length - 1 : 999999).next().value;
...
function index_phase1(state) {
	...
	return [
		...
		4,
		5
	].map((i) => i + 6 * w.indexOf(i) + 72 * w.indexOf(i + 6) + 864 * c);
}
...
var phase1_coord_to_score = new Int8Array(864 * 3);
...
function index_phase2(state) {
	...
	return [ep + 360 * state.cp.indexOf(0), eo + 32 * co];
}
var tables = {};
function generate_phase1_pairing_mtable() {
	...
	for (let midge = 0; midge < 6; midge++) for (let wingl = 0; wingl < 12; wingl++) for (let wingh = 0; wingh < 12; wingh++) {
...
function generate_phase1_pairing2c_ptable() {
	...
	let mtable = generate_phase1_pairingc_mtable();
	let ptable = new Int8Array(864 ** 2 * 3);
	ptable.fill(-1);
	let g = [
		0,
		1,
...
function generate_phase2_permutation_mtable() {
	...
	for (let ep = 0; ep < 360; ep++) {
		...
		for (let m = 0; m < N_MOVES_PHASE2; m++) {
			let new_ep = evenpermutation_to_index(compose(perm, moves[m].mp).slice(0, 6).map((x) => x % 6));
	...
	for (let eo = 0; eo < 32; eo++) {
		...
		for (let co = 0; co < 81; co++) {
			let co_array = [
				...
				2,
				3
			].map((i) => Math.floor(co / 3 ** i) % 3);
			for (let m = 0; m < N_MOVES_PHASE2; m++) {
				let new_eo_array = compose(perm, moves[m].mp).slice(0, 5).map((x) => +(x >= 6));
				let new_eo = 0;
...
//#endregion
export { randomMasterTetraminxScrambleString };
