import { c as QuantumMove, o as Move } from "./chunk-O6HEZXGY.js";
//#region ../core/node_modules/cubing/dist/lib/cubing/puzzle-geometry/index.js
...
var FaceNameSwizzler = class {
	constructor(facenames, gripnames_arg) {
		...
		for (let i = 0; this.prefixFree && i < facenames.length; i++) for (let j = 0; this.prefixFree && j < facenames.length; j++) if (i !== j && facenames[i].startsWith(facenames[j])) this.prefixFree = false;
	}
	prefixFree = true;
	gripnames = [];
	setGripNames(names) {
	...
	spinmatch(userinput, longname) {
		...
		try {
			...
			for (let i = 0; i < e1.length; i++) {
				for (let j = 0; j < i; j++) if (e1[i] === e1[j]) return false;
				let found = false;
				for (let j = 0; j < e2.length; j++) if (e1[i] === e2[j]) {
					found = true;
					break;
				}
...
var FaceRenamingMapper = class {
	...
	convert(move, a, b) {
		...
		else return new Move(new QuantumMove(ngrip, move.innerLayer, move.outerLayer), move.amount);
	}
	notationToInternal(move) {
		return this.convert(move, this.externalNames, this.internalNames);
	}
...
var FTONotationMapper = class {
	...
	notationToInternal(move) {
		if (move.family === "T" && move.innerLayer === void 0 && move.outerLayer === void 0) return new Move(new QuantumMove("FLRv", move.innerLayer, move.outerLayer), move.amount);
		else return this.child.notationToInternal(move);
	}
	notationToExternal(move) {
		let fam = move.family;
		if (fam.length > 0 && fam[fam.length - 1] === "v") fam = fam.substring(0, fam.length - 1);
		if (this.sw.spinmatch(fam, "FLUR")) return new Move(new QuantumMove("T", move.innerLayer, move.outerLayer), move.amount);
	...
	notationToInternal(move) {
		if (move.innerLayer === void 0 && move.outerLayer === void 0) {
			if (Math.abs(move.amount) === 1) {
				if (move.family === "R++") return new Move(new QuantumMove("L", 3, 2), -2 * move.amount);
				else if (move.family === "R--") return new Move(new QuantumMove("L", 3, 2), 2 * move.amount);
				else if (move.family === "D++") return new Move(new QuantumMove("U", 3, 2), -2 * move.amount);
				else if (move.family === "D--") return new Move(new QuantumMove("U", 3, 2), 2 * move.amount);
				if (move.family === "R_PLUSPLUS_") return new Move(new QuantumMove("L", 3, 2), -2 * move.amount);
				else if (move.family === "D_PLUSPLUS_") return new Move(new QuantumMove("U", 3, 2), -2 * move.amount);
			}
			if (move.family === "y") return new Move("Uv", move.amount);
			if (move.family === "x" && Math.abs(move.amount) === 2) return new Move("ERv", move.amount / 2);
		}
		return this.child.notationToInternal(move);
	}
	notationToExternal(move) {
		if (move.family === "ERv" && Math.abs(move.amount) === 1) return new Move(new QuantumMove("x", move.innerLayer, move.outerLayer), move.amount * 2);
		if (move.family === "ILv" && Math.abs(move.amount) === 1) return new Move(new QuantumMove("x", move.innerLayer, move.outerLayer), -move.amount * 2);
		if (move.family === "Uv") return new Move(new QuantumMove("y", move.innerLayer, move.outerLayer), move.amount);
		if (move.family === "Dv") return new Move("y", -move.amount);
		return this.child.notationToExternal(move);
	}
};
...
var NxNxNCubeMapper = class {
	...
	notationToInternal(move) {
		...
		if (!(move.innerLayer || move.outerLayer)) {
			if (grip === "x") move = new Move("Rv", move.amount);
			else if (grip === "y") move = new Move("Uv", move.amount);
			else if (grip === "z") move = new Move("Fv", move.amount);
			if ((this.slices & 1) === 1) {
				if (grip === "E") move = new Move(new QuantumMove("D", (this.slices + 1) / 2), move.amount);
				else if (grip === "M") move = new Move(new QuantumMove("L", (this.slices + 1) / 2), move.amount);
				else if (grip === "S") move = new Move(new QuantumMove("F", (this.slices + 1) / 2), move.amount);
			}
			if (this.slices > 2) {
				if (grip === "e") move = new Move(new QuantumMove("D", this.slices - 1, 2), move.amount);
				else if (grip === "m") move = new Move(new QuantumMove("L", this.slices - 1, 2), move.amount);
	...
	notationToExternal(move) {
		const grip = move.family;
		if (!(move.innerLayer || move.outerLayer)) {
			if (grip === "Rv") return new Move("x", move.amount);
			else if (grip === "Uv") return new Move("y", move.amount);
			else if (grip === "Fv") return new Move("z", move.amount);
			else if (grip === "Lv") return new Move("x", -move.amount);
			else if (grip === "Dv") return new Move("y", -move.amount);
			else if (grip === "Bv") return new Move("z", -move.amount);
		}
		return move;
	}
};
...
var PyraminxNotationMapper = class {
	...
	notationToInternal(move) {
		if (this.wcaHack && move.innerLayer === 2 && move.outerLayer === null) {
			...
			if (newFamilyWCA) return new Move(new QuantumMove(newFamilyWCA, move.innerLayer, move.outerLayer), move.amount);
		}
		const newFamily = this.map[move.family];
		if (newFamily) return new Move(new QuantumMove(newFamily, move.innerLayer, move.outerLayer), move.amount);
		else if (pyraminxExternalQuantumY.isIdentical(move.quantum)) return new Move(pyraminxInternalQuantumY, -move.amount);
		else return null;
	}
	notationToExternal(move) {
		if (this.wcaHack && move.innerLayer === 2 && move.outerLayer === null) {
			for (const [external, internal] of Object.entries(pyraminxFamilyMapWCA)) if (this.child.spinmatch(move.family, internal)) return new Move(new QuantumMove(external, move.innerLayer, move.outerLayer), move.amount);
		}
		for (const [external, internal] of Object.entries(this.map)) if (this.child.spinmatch(move.family, internal)) return new Move(new QuantumMove(external, move.innerLayer, move.outerLayer), move.amount);
		if (pyraminxInternalQuantumY.isIdentical(move.quantum)) return new Move(pyraminxExternalQuantumY, -move.amount);
		else return null;
	}
};
...
var SkewbNotationMapper = class {
	...
	notationToInternal(move) {
		...
		if (newFamily) return new Move(new QuantumMove(newFamily, move.outerLayer, move.innerLayer), move.amount);
		if (skewbExternalQuantumX.isIdentical(move.quantum)) return new Move(skewbInternalQuantumX, move.amount);
		if (skewbExternalQuantumY.isIdentical(move.quantum)) return new Move(skewbInternalQuantumY, move.amount);
		if (skewbExternalQuantumZ.isIdentical(move.quantum)) return new Move(skewbInternalQuantumZ, move.amount);
		return null;
	}
	notationToExternal(move) {
		for (const [external, internal] of Object.entries(skewbFamilyMap)) if (this.child.spinmatchv(move.family, internal)) return new Move(new QuantumMove(external, move.innerLayer, move.outerLayer), move.amount);
		if (skewbInternalQuantumX.isIdentical(move.quantum)) return new Move(skewbExternalQuantumX, move.amount);
		if (skewbInternalQuantumXPrime.isIdentical(move.quantum)) return new Move(skewbExternalQuantumX, -move.amount);
		if (skewbInternalQuantumY.isIdentical(move.quantum)) return new Move(skewbExternalQuantumY, move.amount);
		if (skewbInternalQuantumYPrime.isIdentical(move.quantum)) return new Move(skewbExternalQuantumY, -move.amount);
		if (skewbInternalQuantumZ.isIdentical(move.quantum)) return new Move(skewbExternalQuantumZ, move.amount);
		if (skewbInternalQuantumZPrime.isIdentical(move.quantum)) return new Move(skewbExternalQuantumZ, -move.amount);
		return null;
	}
};
function remapKPuzzleDefinition(internalDefinition, notationMapper) {
	...
	for (const [internalMoveName, transformationData] of Object.entries(internalDefinition.moves)) {
		...
		const externalPrefix = notationMapper.notationToExternal(Move.fromString(prefix));
		if (!externalPrefix) continue;
		const externalMoveName = externalPrefix + suffix;
		if (!externalMoveName) throw new Error(`Missing external move name for: ${internalMoveName.toString()}`);
...
var PuzzleGeometryFullOptions = class {
	...
	scrambleAmount = 0;
	constructor(options = {}) {
		const optionsWithoutUndefined = Object.fromEntries(Object.entries(options).filter(([_, value]) => typeof value !== "undefined"));
...
var Perm = class _Perm {
	...
	compareTo(p2) {
		for (let i = 0; i < this.n; i++) if (this.p[i] !== p2.p[i]) return this.p[i] - p2.p[i];
		return 0;
	}
	toGap() {
		const cyc = [];
...
var PGOrbitsDef = class _PGOrbitsDef {
	...
	toKTransformationData(t) {
		...
		for (let i = 0; i < this.orbitnames.length; i++) ktransformationData[this.orbitnames[i]] = t.orbits[i].toKTransformationOrbitData();
		return ktransformationData;
	}
	toKPatternData(t) {
		const kpatternData = {};
		for (let i = 0; i < this.orbitnames.length; i++) kpatternData[this.orbitnames[i]] = t.orbits[i].toKPatternOrbitData();
		return kpatternData;
	}
	static transformToKTransformationData(orbitnames, t) {
		const mp = {};
		for (let j = 0; j < orbitnames.length; j++) mp[orbitnames[j]] = t.orbits[j].toKTransformationOrbitData();
		return mp;
	}
	...
	toKsolve(name, mapper = new NullMapper()) {
		...
		result.push("");
		result.push("Solved");
		for (let i = 0; i < this.orbitnames.length; i++) this.solved.orbits[i].appendDefinition(result, this.orbitnames[i], false, false);
		result.push("End");
		for (let i = 0; i < this.movenames.length; i++) {
			result.push("");
			let name2 = this.movenames[i];
			if (!this.forcenames[i]) name2 = externalName(mapper, this.movenames[i]);
			let doinv = false;
			...
			for (let j = 0; j < this.orbitnames.length; j++) if (doinv) this.moveops[i].orbits[j].inv().appendDefinition(result, this.orbitnames[j], true);
			else this.moveops[i].orbits[j].appendDefinition(result, this.orbitnames[j], true);
			result.push("End");
		}
		return result;
	}
	toKPuzzleDefinition(includemoves) {
		...
		const moves = {};
		if (includemoves) for (let i = 0; i < this.movenames.length; i++) moves[this.movenames[i]] = this.toKTransformationData(this.moveops[i]);
		return {
	...
	optimize() {
		...
		for (let i = 0; i < this.orbitdefs.length; i++) {
			...
			for (let j = 0; j < this.moveops.length; j++) for (let k = 0; k < n; k++) if (this.moveops[j].orbits[i].perm[k] !== k || this.moveops[j].orbits[i].ori[k] !== 0) {
				if (!this.isRotation[j]) changed[k] = true;
			...
			if (om > 1) {
				...
				for (let j = 0; j < this.moveops.length; j++) for (let k = 0; k < n; k++) if (this.moveops[j].orbits[i].perm[k] !== k || this.moveops[j].orbits[i].ori[k] !== 0) for (let o = 0; o < om; o++) duo.union(k * om + o, this.moveops[j].orbits[i].perm[k] * om + (o + this.moveops[j].orbits[i].ori[k]) % om);
				for (let j = 0; !keepori && j < n; j++) for (let o = 1; o < om; o++) if (duo.find(j * om) === duo.find(j * om + o)) keepori = true;
				for (let j = 0; !keepori && j < n; j++) for (let k = 0; k < j; k++) if (this.solved.orbits[i].perm[j] === this.solved.orbits[i].perm[k]) keepori = true;
			}
			let nontriv = -1;
			let multiple = false;
			for (let j = 0; j < this.orbitdefs[i].size; j++) if (changed[j]) {
				const h = du.find(j);
			...
			for (let j = 0; j < this.orbitdefs[i].size; j++) {
				...
				if (du.find(j) !== j) continue;
				const no = [];
				const on = [];
				let nv = 0;
				for (let k = 0; k < this.orbitdefs[i].size; k++) if (du.find(k) === j) {
					no[nv] = k;
					on[k] = nv;
					nv++;
				}
				if (multiple) neworbitnames.push(`${this.orbitnames[i]}_p${j}`);
		...
		return new _PGOrbitsDef(neworbitnames, neworbitdefs, new VisibleState(newsolved), this.movenames, newmoveops.map((_) => new PGTransform(_)), this.isRotation, this.forcenames);
	}
	scramble(n) {
	...
	getScrambleTransformation(n) {
		...
		for (let i = 0; i < n; i++) {
			...
			if (Math.random() < .1) pool[ri] = pool[ri].mul(this.moveops[rm]);
		}
		let s = pool[0];
		for (let i = 1; i < pool.length; i++) s = s.mul(pool[i]);
		return s;
	}
	reassemblySize() {
		let n = BigInt(1);
...
var PGOrbit = class _PGOrbit {
	...
	equal(b) {
		const n = this.perm.length;
		for (let i = 0; i < n; i++) if (this.perm[i] !== b.perm[i] || this.ori[i] !== b.ori[i]) return false;
		return true;
	}
	killOri() {
		const n = this.perm.length;
	...
	isIdentity() {
		...
		for (let i = 0; i < n; i++) if (this.perm[i] !== i || this.ori[i] !== 0) return false;
		return true;
	}
	zeroOris() {
		const n = this.perm.length;
		if (this.ori === zeros(n)) return true;
		for (let i = 0; i < n; i++) if (this.ori[i] !== 0) return false;
		return true;
	}
	remap(no, on, nv) {
	...
	toKTransformationOrbitData() {
		const n = this.perm.length;
		if (this.isIdentity()) {
			if (!_PGOrbit.ktransformationCache[n]) _PGOrbit.ktransformationCache[n] = {
				permutation: iota(n),
...
var PGTransformBase = class {
	...
	equal(b) {
		for (let i = 0; i < this.orbits.length; i++) if (!this.orbits[i].equal(b.orbits[i])) return false;
		return true;
	}
	killOri() {
...
var PGTransform = class _PGTransform extends PGTransformBase {
	...
	e() {
		return new _PGTransform(this.orbits.map((_) => PGOrbit.e(_.perm.length, _.orimod)));
	}
};
...
var Quat = class _Quat {
	...
	mul(q) {
		return new _Quat(this.a * q.a - this.b * q.b - this.c * q.c - this.d * q.d, this.a * q.b + this.b * q.a + this.c * q.d - this.d * q.c, this.a * q.c - this.b * q.d + this.c * q.a + this.d * q.b, this.a * q.d + this.b * q.c - this.c * q.b + this.d * q.a);
	}
	toString() {
	...
	cross(q) {
		return new _Quat(0, this.c * q.d - this.d * q.c, this.d * q.b - this.b * q.d, this.b * q.c - this.c * q.b);
	}
	dot(q) {
	...
	orthogonal() {
		...
		if (ab < ac && ab < ad) return this.cross(new _Quat(0, 1, 0, 0)).normalize();
		else if (ac < ab && ac < ad) return this.cross(new _Quat(0, 0, 1, 0)).normalize();
		else return this.cross(new _Quat(0, 0, 0, 1)).normalize();
	}
	pointrotation(b) {
		const a = this.normalize();
		b = b.normalize();
		if (a.sub(b).len() < eps) return new _Quat(1, 0, 0, 0);
		let h = a.sum(b);
	...
	intersect3(p2, p3) {
		...
		return new _Quat(0, this.det3x3(this.a, this.c, this.d, p2.a, p2.c, p2.d, p3.a, p3.c, p3.d) / det, this.det3x3(this.b, this.a, this.d, p2.b, p2.a, p2.d, p3.b, p3.a, p3.d) / det, this.det3x3(this.b, this.c, this.a, p2.b, p2.c, p2.a, p3.b, p3.c, p3.a) / det);
	}
	side(x) {
		if (x > eps) return 1;
		if (x < -eps) return -1;
			...
			for (let s = -1; s <= 1; s += 2) {
				...
				for (let k = 0; k < face.length; k++) {
					...
					if (inout[k] + inout[kk] === 0 && inout[k] !== 0) {
						...
						const r2 = vk / (vk - (face[kk].dot(this) - d));
...
function tetrahedron() {
	return [new Quat(.5, .5, .5, .5), new Quat(.5, .5, .5, -.5)];
}
function dodecahedron() {
	const d36 = 2 * Math.PI / 10;
	let dx = .5 + .3 * Math.sqrt(5);
	let dy = .5 + .1 * Math.sqrt(5);
	const dd = Math.sqrt(dx * dx + dy * dy);
	dx /= dd;
	dy /= dd;
	return [new Quat(Math.cos(d36), dx * Math.sin(d36), dy * Math.sin(d36), 0), new Quat(.5, .5, .5, .5)];
}
function icosahedron() {
	...
	return [new Quat(Math.cos(ang), dx * Math.sin(ang), dy * Math.sin(ang), 0), new Quat(Math.cos(ang), -dx * Math.sin(ang), dy * Math.sin(ang), 0)];
}
function octahedron() {
	const s5 = Math.sqrt(.5);
	return [new Quat(.5, .5, .5, .5), new Quat(s5, 0, 0, s5)];
}
function closure(g) {
...
var FactoredNumber = class {
	...
	multiply(n) {
		...
		if (n > 1) if (void 0 !== this.mult[n]) this.mult[n]++;
		else this.mult[n] = 1;
	}
	toString() {
		let r = "";
...
function findelement(a, p) {
	for (let i = 0; i < a.length; i++) if (a[i][0].dist(p) < eps3) return i;
...
function getPuzzleGeometryByDesc(desc, options = {}) {
	...
	if (parsed === null) throw Error("Could not parse the puzzle description");
...
function getmovename(geo, bits, slices) {
	...
	} else throw Error(`We only support slice and outer block moves right now. ${bits}`);
...
function splitByFaceNames(s, facenames) {
	...
	while (at < s.length) {
		...
		} else throw Error(`Could not split ${s} into face names.`);
	}
	return r;
}
...
var PuzzleGeometry = class {
	constructor(puzzleDescription, options) {
		...
		switch (shape) {
			...
			case "d":
				g = dodecahedron();
				break;
			default: throw Error(`Bad shape argument: ${shape}`);
		}
		...
		const baseplanes = this.basePlaneRotations.map((_) => baseplane.rotateplane(_));
		...
		const firstface = getface(uniqueplanes(boundary, this.rotations).map((_) => boundary.rotateplane(_)));
		this.edgeDistance = firstface[0].sum(firstface[1]).smul(.5).dist(zero);
		...
		for (const cut of cuts) {
			...
			switch (cut.cutType) {
				...
				case "e":
					...
					sawedge = true;
					break;
				default: throw Error(`Bad cut argument: ${cut.cutType}`);
			}
		...
		for (let i = 0; i < this.basePlaneRotations.length; i++) {
			...
			for (let j = 0; j < face.length; j++) {
				...
				searchaddelement(edgenames, face[j].sum(face[jj]).smul(.5), i);
			}
		}
		const otherfaces = [];
		for (let i = 0; i < this.basePlaneRotations.length; i++) {
			...
			for (let j = 0; j < face.length; j++) {
				const jj = (j + 1) % face.length;
				const el = edgenames[findelement(edgenames, face[j].sum(face[jj]).smul(.5))];
				if (i === el[1]) facelist.push(el[2]);
				else if (i === el[2]) facelist.push(el[1]);
				else throw Error("Could not find edge");
			}
		...
		for (const neti of net) {
			const fi = facenametoindex[neti[0]];
			if (fi === void 0) throw Error("Bad edge description; first edge not connected");
			let ii = -1;
			...
			if (ii < 0) throw Error("First element of a net not known");
			for (let j = 2; j < neti.length; j++) {
				if (neti[j] === "") continue;
				const of = otherfaces[fi][(j + ii - 1) % edgesperface];
				const fn2 = faceindextoname[of];
				if (fn2 !== void 0 && fn2 !== neti[j]) throw Error("Face mismatch in net");
		...
		for (let i = 0; i < this.basePlaneRotations.length; i++) {
			...
			for (let j = 0; j < face.length; j++) {
				const jj = (j + 1) % face.length;
				const midpoint = face[j].sum(face[jj]).smul(.5);
				const jjj = (j + 2) % face.length;
				const midpoint2 = face[jj].sum(face[jjj]).smul(.5);
		...
		const markedface = [];
		for (let i = 0; i < this.baseFaceCount; i++) markedface[1 << i] = i;
		{
		...
		for (let i = 0; i < edgenames.length; i++) {
			if (edgenames[i].length !== 3) throw Error(`Bad length in edge names ${edgenames[i]}`);
			const f1 = edgenames[i][1];
			const f2 = edgenames[i][2];
			let c1 = faceindextoname[f1];
			const c2 = faceindextoname[f2];
			if (markedface[1 << f1 | 1 << f2] === f1) c1 = c1 + sep + c2;
			else c1 = c2 + sep + c1;
			edgenames[i] = [edgenames[i][0], c1];
		}
		for (let i = 0; i < vertexnames.length; i++) {
			let bits = 0;
			if (vertexnames[i].length < 4) throw Error("Bad length in vertex names");
			for (let j = 1; j < vertexnames[i].length; j++) bits |= 1 << facenametoindex[vertexnames[i][j][0]];
			const fi = markedface[bits];
			let st = -1;
			for (let j = 1; j < vertexnames[i].length; j++) if (fi === facenametoindex[vertexnames[i][j][0]]) st = j;
			if (st < 0) throw Error("Internal error; couldn't find face name when fixing corners");
			let r = "";
			for (let j = 1; j < vertexnames[i].length; j++) {
				if (j === 1) r = vertexnames[i][st][0];
				else r = r + sep + vertexnames[i][st][0];
				for (let k = 1; k < vertexnames[i].length; k++) if (vertexnames[i][st][1] === vertexnames[i][k][2]) {
					st = k;
					break;
				}
			}
		...
		for (let c = 0; c < cutplanes.length; c++) for (const rotation of this.rotations) {
			...
			if (!wasseen) {
				...
				if (intersects[c]) this.movePlanesFiltered.push(q);
			}
		}
		...
		for (const rotation of this.rotations) {
			...
			if (cm.dist(centermassface(f)) < eps3) simplerot.push(rotation);
		}
		...
		for (const face of faces) for (let j = 0; j < face.length; j++) {
			...
			if (t < this.shortestEdge) this.shortestEdge = t;
		}
		if (this.options.verbosity > 0) console.log(`# Short edge is ${this.shortestEdge}`);
	...
	project2d(facen, edgen, targvec) {
		...
		return [
			x1,
			y1,
			new Quat(0, targvec[0].b - x1.dot(face[edgen]), targvec[0].c - y1.dot(face[edgen]), 0)
		];
	}
	upperStringToBitSet(geo) {
		let r = 0;
		for (let i = 0; i < geo.length; i++) r |= 1 << geo.charCodeAt(i) - 65;
		return r;
	}
	allstickers() {
		...
		const moverotations = [];
		for (let i = 0; i < moveplanesets.length; i++) moverotations.push([]);
		for (const q of this.rotations) {
			if (Math.abs(Math.abs(q.a) - 1) < eps3) continue;
			const qnormal = q.makenormal();
		...
		for (let i = 0; i < moverotations.length; i++) {
			...
			if (moverotations[i][0].dot(moveplanenormals[i]) < 0) r.reverse();
		}
		...
		for (let i = 0; i < moveplanesets.length; i++) {
			...
			if (pos === null || neg === null) throw Error("Saw positive or negative sides as null");
			movesetgeos.push([
				pos[0],
				pos[1],
				neg[0],
			...
			if (this.addNotationMapper === "PyraminxOrTetraminxMapper") if (moveplanesets[0].length === 2 && moveplanesets[0][0].a === .333333333333333 && moveplanesets[0][1].a === 1.66666666666667) {
			...
			if (this.addNotationMapper === "MegaminxMapper" && gtype === "f") {
				if (1 + moveplanesets[i].length === 3) this.notationMapper = new MegaminxScramblingNotationMapper(this.notationMapper);
				this.addNotationMapper = "";
			}
			if (this.addNotationMapper === "FTOMapper" && gtype === "f") {
				if (1 + moveplanesets[i].length === 3) this.notationMapper = new FTONotationMapper(this.notationMapper, this.swizzler);
				this.addNotationMapper = "";
			}
		}
		...
		for (let i = 0; i < faces.length; i++) {
			...
			if (!facelisthash.get(s)) facelisthash.set(s, [i]);
			else {
		...
		for (const facelist of facelisthash.values()) {
			...
			if (facelist.length > 1) {
				...
				for (let looplimit = 0; facelist.length > 2; looplimit++) {
					...
					if (!changed) break;
					if (looplimit > 1e3) throw Error("Bad epsilon math; too close to border");
				}
				let bits = 0;
				...
				if (mini < 0) throw Error("Could not find marked face in list");
				if (mini !== 0) {
		...
		if (this.setReidOrSpeffzOrder && 4 <= this.stickersPerFace) {
			...
			const reidmap = {};
			for (const cubie of reidorder) for (let j = 0; j < cubie.length; j++) reidmap[this.upperStringToBitSet(cubie[j])] = j;
			const cornerloc = {};
			...
			if (this.stickersPerFace > 9) {
				for (const vertex of this.vertexNames) cornerloc[this.upperStringToBitSet(vertex[1])] = vertex[0];
				for (let i = 0; i < 6; i++) spefffacelookup[spefffaceorder[i]] = i;
				for (const co of speffcornerorder) speffcornerlocs.push(cornerloc[this.upperStringToBitSet(co)]);
			}
			for (const cubieset of cubiesetcubies) for (const cubienum of cubieset) if (cubies[cubienum].length === 3 || this.stickersPerFace <= 9) {
				let mask = 0;
			...
			} else if (cubies[cubienum].length <= 2) {
				const ordset = [];
				for (let k = 0; k < cubies[cubienum].length; k++) {
					...
					let bestdist = 0x56bc75e2d63100000;
					...
					for (let i = 0; i < 4; i++) {
						...
						if (t + eps3 < bestdist) {
							bestdist = t;
							bestmask = 1 << i;
						} else if (t < bestdist + eps3) bestmask |= 1 << i;
					}
				...
				if (ordset.length > 0) {
					if (cubies[cubienum].length === 1) cubieordnums[cubienum] = ordset[0][0];
						...
						while (k < 3 && (f0c === corn.charAt(k) || f1c === corn.charAt(k))) k++;
						if (k === 3) throw Error("Internal error (2) in Speffz");
						k = (k + 1) % 3;
						if (f0c === corn.charAt(k)) cubieordnums[cubienum] = ordset[0][0];
						else if (f1c === corn.charAt(k)) cubieordnums[cubienum] = ordset[1][0];
						else {
		...
		if (this.options.fixedPieceType !== null) {
			...
			if (this.fixedCubie < 0) throw Error(`Could not find a cubie of type ${this.options.fixedPieceType} to fix.`);
		}
	...
	stringToBlockMove(mv) {
		const p = mv.match(/^(([0-9]+)-)?([0-9]+)?([^0-9]+)([0-9]+'?)?$/);
		if (p === null) throw Error(`Bad move passed ${mv}`);
		const grip = p[4];
		let loslice;
		let hislice;
		if (p[2] !== void 0) {
			if (p[3] === void 0) throw Error("Missing second number in range");
	...
	parseMove(move) {
		...
		if (bm === null) throw Error(`Bad move ${move.family}`);
		move = bm;
		let grip = move.family;
		let fullrotation = false;
		if (grip.endsWith("v") && grip[0] <= "Z") {
			if (move.innerLayer !== void 0 || move.outerLayer !== void 0) throw Error("Cannot use a prefix with full cube rotations");
			grip = grip.slice(0, -1);
			fullrotation = true;
		}
		...
		if (geo === void 0) throw Error(`Bad grip in move ${move.family}`);
		if (move.outerLayer !== void 0) loslice = move.outerLayer;
		if (move.innerLayer !== void 0) if (move.outerLayer === void 0) {
		...
		if (loslice < 0 || loslice > movePlaneSets[msi].length || hislice < 0 || hislice > movePlaneSets[msi].length) throw Error(`Bad slice spec ${loslice} ${hislice} vs ${movePlaneSets[msi].length}`);
		if (!permissivieMoveParsing && loslice === 0 && hislice === movePlaneSets[msi].length && !fullrotation) throw Error("! full puzzle rotations must be specified with v suffix.");
		return [
			void 0,
			msi,
			loslice,
	...
	parsemove(mv) {
		if (mv instanceof Move) mv = mv.toString();
		...
		if (this.options.orientCenters) {
			for (let k = 0; k < this.cubies.length; k++) if (this.cubies[k].length === 1) {
				...
				if (center.dist(this.faceCenterMass[kk]) < eps3) {
					...
					if (Math.abs(normal.dot(this.faces[kk].get(hii2).sub(center)) - hiv) < eps3) hii = hii2;
					if (hii !== 0) {
						const qs = [];
		...
		for (let k = 0; k < this.movePlaneSets.length; k++) {
			...
			for (let i = 0; i < this.faces.length; i++) {
				...
				if (b.length > 2 && this.options.orientCenters && (this.cubies[b[0]].length === 1 || this.duplicatedCubies[b[0]] > 1)) {
					if (this.faceCenterMass[i].dist(this.baseFaces[this.getfaceindex(i)].centermass()) < eps3) {
						...
						for (let ii = 0; ii < b.length; ii += 2) {
							...
							if (o < 0) throw Error("Couldn't find rotation of center faces; ignoring for now.");
							else {
								b[ii + 1] = o;
				...
				if (b.length === 2 && this.options.orientCenters) {
					...
					for (let ii = 1; ii < this.movesetorders[k]; ii++) if (dir > 0) b.push(b[0], ii);
					else b.push(b[0], (this.movesetorders[k] - ii) % this.movesetorders[k]);
				}
				if (b.length > 2 && !cubiedone[b[0]]) {
					if (b.length !== 2 * this.movesetorders[k]) throw Error("Bad length in perm gen");
	...
	getmovesets(k) {
		...
		if (this.parsedMoveList !== void 0) for (const parsedmove of this.parsedMoveList) {
			...
			if (parsedmove[4]) r.push([parsedmove[2], parsedmove[3]]);
			else r.push([slices - parsedmove[3], slices - parsedmove[2]]);
			r.push(parsedmove[5]);
		}
		else {
			...
			if (this.options.vertexMoves && isTetrahedron && !this.options.allMoves) {
				if (msg[1] !== msg[3]) for (let i = 0; i < slices; i++) if (msg[1] !== "v") {
					if (this.options.outerBlockMoves) r.push([i + 1, slices]);
					else r.push([i + 1, i + 1]);
					r.push(1);
				} else {
					if (this.options.outerBlockMoves) r.push([0, i]);
					else r.push([i, i]);
					r.push(1);
				}
			} else for (let i = 0; i <= slices; i++) {
				if (!this.options.allMoves && i + i === slices) continue;
				if (this.options.outerBlockMoves) if (i + i > slices) r.push([i, slices]);
				else r.push([0, i]);
				else r.push([i, i]);
				r.push(1);
			}
		...
		if (this.fixedCubie >= 0) {
			...
			for (let i = 0; i < r.length; i += 2) {
				let o = r[i];
				if (dep >= o[0] && dep <= o[1]) if (o[0] === 0) o = [o[1] + 1, slices];
				else if (slices === o[1]) o = [0, o[0] - 1];
...
`;
	...
	writegap() {
		...
		for (let i = 0; i < os.moveops.length; i++) {
			...
			if (!os.forcenames[i]) movename = `M_${externalName(this.notationMapper, movename)}`;
	...
	writemathematica() {
		...
		for (let i = 0; i < os.moveops.length; i++) {
			...
			mvs.push(movename);
			if (doinv) r.push(`${movename}=${os.moveops[i].toPerm().inv().toMathematica()};`);
			else r.push(`${movename}=${os.moveops[i].toPerm().toMathematica()};`);
		}
		r.push(`gen={${mvs.join(",")}};`);
		return r.join("\n");
	}
	...
	getKPuzzleDefinition(fortwisty = true, includemoves = true) {
		const internalDefinition = this.getOrbitsDef(fortwisty, includemoves).toKPuzzleDefinition(includemoves);
		internalDefinition.experimentalPuzzleDescription = this.puzzleDescription;
		if (!internalDefinition) throw Error("Missing definition!");
	...
	getMoveFromBits(moverange, amount, inverted, axiscmoves, setmoves, movesetorder) {
		...
		for (let m = moverange[0]; m <= moverange[1]; m++) {
			...
			for (let j = 0; j < slicecmoves.length; j += 2 * movesetorder) {
				...
				if (perms[setnum] === iota(this.cubieOrbitSizes[setnum])) {
					...
					if (this.orbitOrientations[setnum] > 1 && !this.options.fixedOrientation) oris[setnum] = oris[setnum].slice();
				}
				for (let ii = 0; ii < mperm.length; ii += 2) {
					perms[setnum][mperm[(ii + inc) % mperm.length]] = mperm[ii];
					if (this.orbitOrientations[setnum] > 1 && !this.options.fixedOrientation) oris[setnum][mperm[ii]] = (mperm[(ii + oinc) % mperm.length] - mperm[(ii + 1) % mperm.length] + 2 * this.orbitOrientations[setnum]) % this.orbitOrientations[setnum];
				}
			}
		}
		...
		for (let ii = 0; ii < this.cubieSetNames.length; ii++) {
			...
			if (this.orbitOrientations[ii] === 1 || this.options.fixedOrientation) if (perms[ii] === iota(lastId.perm.length)) {
				...
				moveorbits.push(new PGOrbit(perms[ii], no, this.orbitOrientations[ii]));
			}
		}
	...
	omitSet(name) {
		for (const excludedSet of this.options.excludeOrbits) if (excludedSet === name) return true;
		return false;
	}
	diffmvsets(a, b, slices, neg) {
		for (let i = 0; i < a.length; i += 2) {
			let found = false;
			for (let j = 0; !found && j < b.length; j += 2) if (neg) {
	...
	getOrbitsDef(fortwisty, includemoves = true) {
		...
		if (this.options.addRotations && (this.options.moveList || this.options.fixedPieceType !== null)) {
			...
			for (let k = 0; k < this.movePlaneSets.length; k++) {
				...
				for (let i = 0; i < this.moveRotations.length; i++) {
					...
					for (let ii = 1; ii * 2 <= this.movesetorders[i]; ii++) {
						...
						let found = -1;
						let neg = false;
						for (let j = 0; j < this.movePlaneNormals.length; j++) if (nn.dist(this.movePlaneNormals[j]) < eps3) {
							found = j;
							break;
						...
						if (found < 0) throw Error("Could not find rotation");
						const cmp = mps[found];
			...
			for (let i = 0; i < this.moveRotations.length; i++) if (addrot[i] === 0) addrot[i] = 1;
			else if (addrot[i] === 1) if (this.movesetorders[i] > 3) addrot[i] = 2;
			else addrot[i] = 0;
			else if (addrot[i] === 3) addrot[i] = 0;
			else throw Error("Impossible addrot val");
		}
		for (let k = 0; k < this.movePlaneSets.length; k++) if (addrot[k] !== 0 && !hasrotation[k]) {
		...
		for (let k = 0; k < this.movePlaneSets.length; k++) {
			...
			for (let i = 0; i < moveset.length; i += 2) for (let j = 0; j < i; j += 2) if (moveset[i][0] === moveset[j][0] && moveset[i][1] === moveset[j][1]) throw Error("Redundant moves in moveset.");
			const allbits = [];
			for (let i = 0; i < moveset.length; i += 2) for (let j = moveset[i][0]; j <= moveset[i][1]; j++) allbits[j] = 1;
		...
		for (let i = 0; i < this.cubieSetNames.length; i++) {
			if (!setmoves[i]) continue;
			if (this.omitSet(this.cubieSetNames[i])) {
				setmoves[i] = 0;
				continue;
			}
			setnames.push(this.cubieSetNames[i]);
			setdefs.push(new PGOrbitDef(this.cubieOrbitSizes[i], this.options.fixedOrientation ? 1 : this.orbitOrientations[i]));
		}
		const solved = [];
		for (let i = 0; i < this.cubieSetNames.length; i++) {
			if (!setmoves[i]) continue;
			if (this.omitSet(this.cubieSetNames[i])) continue;
			...
			solved.push(new PGOrbit(p, o, this.options.fixedOrientation ? 1 : this.orbitOrientations[i]));
		}
		const movenames = [];
		const forcenames = [];
		const moves = [];
		const isrots = [];
		if (includemoves) for (let k = 0; k < this.movePlaneSets.length; k++) {
			const slices = this.movePlaneSets[k].length;
			const moveset = mps[k];
			...
			for (let i = 0; i < moveset.length; i += 2) {
				...
				if (this.parsedMoveList !== void 0) for (const parsedmove of this.parsedMoveList) {
					if (parsedmove[1] !== k) continue;
					let r2 = [];
					if (parsedmove[4]) r2 = [parsedmove[2], parsedmove[3]];
					else r2 = [slices - parsedmove[3], slices - parsedmove[2]];
	...
	getOrientationRotation(desiredRotation) {
		...
		if (!feature1) throw Error(`Could not find feature ${feature1name}`);
		if (!feature2) throw Error(`Could not find feature ${feature2name}`);
		const r1 = feature1.pointrotation(direction1);
		return feature2.rotatepoint(r1).unproject(direction1).pointrotation(direction2.unproject(direction1)).mul(r1);
	}
	getInitial3DRotation() {
		...
		if (!orientationDescription) throw Error("No default orientation?");
		return this.getOrientationRotation(orientationDescription);
	}
	generate2dmapping(w = 800, h = 500, trim = 10, threed = false, twodshrink = .92) {
		w -= 2 * trim;
		h -= 2 * trim;
		...
		this.genperms();
		const polyn = this.getboundarygeometry().facenames[0][0].length;
		const net = this.net;
		if (net === null) throw Error("No net?");
		const edges = {};
		let minx = 0;
		let miny = 0;
		let maxx = 1;
		...
		for (const neti of net) {
			const f0 = neti[0];
			if (!edges[f0]) throw Error("Bad edge description; first edge not connected.");
		...
		const xoff = .5 * (w - sc * (maxx + minx));
		const yoff = .5 * (h - sc * (maxy + miny));
		const geos = {};
		...
		geos[this.faceNames[0][1]] = this.project2d(0, 0, [new Quat(0, initv[0][0], initv[0][1], 0), new Quat(0, initv[1][0], initv[1][1], 0)]);
		const connectat = [];
		connectat[0] = 0;
		for (const neti of net) {
			const f0 = neti[0];
			if (!edges2[f0]) throw Error("Bad edge description; first edge not connected.");
			let gfi = -1;
			for (let j = 0; j < bg.facenames.length; j++) if (f0 === bg.facenames[j][1]) {
				gfi = j;
				break;
			}
			if (gfi < 0) throw Error(`Could not find first face name ${f0}`);
			...
			for (let j = 1; j < neti.length; j++) {
				...
				const mp = thisface[(caf0 + j) % polyn].sum(thisface[(caf0 + j + polyn - 1) % polyn]).smul(.5);
				...
				if (gf1i < 0) throw Error("Could not find second face name");
				const otherface = bg.facenames[gf1i][0];
				for (let k = 0; k < otherface.length; k++) if (otherface[k].sum(otherface[(k + 1) % polyn]).smul(.5).dist(mp) <= eps3) {
					const p1 = edges2[f0][(j + polyn - 1) % polyn];
					const p2 = edges2[f0][j % polyn];
					connectat[gf1i] = k;
					geos[gf1] = this.project2d(gf1i, k, [new Quat(0, p2[0], p2[1], 0), new Quat(0, p1[0], p1[1], 0)]);
					break;
				}
			}
		}
		...
		const mappt2d = (fn, q) => {
			if (threed) {
				q = q.rotatepoint(rot);
				const xoff2 = .5 * trim + .25 * w;
				const xmul = this.basePlanes[fn].rotateplane(rot).d < 0 ? 1 : -1;
				return [trim + w * .5 + xmul * (xoff2 - q.b * sc2), trim + h * .5 + q.c * sc2];
			} else {
				const g = geos[this.faceNames[fn][1]];
				return [trim + twodshrink * q.dot(g[0]) + g[2].b, trim + h - twodshrink * q.dot(g[1]) - g[2].c];
			}
		};
		return mappt2d;
	}
...
`);
		...
		return `<svg id="svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 800 500">
...
${svg.join("")}</svg>`;
	...
	get3d(options) {
		...
		const rot = this.getInitial3DRotation();
		const faces = [];
		const maxdist = .52 * this.baseFaces[0].get(0).len();
		...
		for (let i = 0; i < this.faces.length; i++) {
			...
			if (options?.stickerColors) color = options.stickerColors[i];
		...
		for (let i = 0; i < this.movesetgeos.length; i++) {
			...
			for (const gn of this.geometryNormals) if (msg[0] === gn[1] && msg[1] === gn[2]) {
				grips.push({
	...
	getGeoNormal(geoname) {
		...
		for (const gn of this.geometryNormals) if (grip === gn[1]) {
...
Vertex distance ${this.vertexDistance}`;
	...
	writeSchreierSims(tw) {
		const as = this.getOrbitsDef(false).reassemblySize();
		tw(`Reassembly size is ${as}`);
		tw(`Ratio is ${as / schreierSims(this.getMovesAsPerms(), tw)}`);
	}
};
var PGNotation = class {
	...
	lookupMove(move) {
		...
		return PGOrbitsDef.transformToKTransformationData(this.orbitNames, pgmv);
	}
...
//#endregion
export { PGNotation as ExperimentalPGNotation, getPuzzleGeometryByDesc, getPuzzleGeometryByName };
