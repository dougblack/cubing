//#region ../core/node_modules/cubing/dist/lib/cubing/chunks/chunk-O6HEZXGY.js
...
var AlgCommon = class extends Comparable {
	constructor() {
		super();
		if (writeAlgDebugField) Object.defineProperty(this, "_debugStr", { get: () => {
...
var Commutator = class _Commutator extends AlgCommon {
	...
	*experimentalExpand(iterDir = 1, depth) {
		depth ??= Infinity;
		if (depth === 0) yield iterDir === 1 ? this : this.invert();
		else if (iterDir === 1) {
			yield* this.A.experimentalExpand(1, depth - 1);
			yield* this.B.experimentalExpand(1, depth - 1);
			yield* this.A.experimentalExpand(-1, depth - 1);
			yield* this.B.experimentalExpand(-1, depth - 1);
		} else {
			yield* this.B.experimentalExpand(1, depth - 1);
...
var algDebugGlobals = { caretNISSNotationEnabled: true };
...
function transferCharIndex(from, to) {
	if (startCharIndexKey in from) to[startCharIndexKey] = from[startCharIndexKey];
	if (endCharIndexKey in from) to[endCharIndexKey] = from[endCharIndexKey];
	return to;
}
var AlgParser = class {
	#input = "";
	...
	parseAlg(input) {
		...
		const { [startCharIndexKey]: startCharIndex, [endCharIndexKey]: endCharIndex } = alg;
	...
	parseAlgWithStopping(stopBefore) {
		...
		while (this.#idx < this.#input.length) {
			...
			if (stopBefore.includes(this.#input[this.#idx])) return addCharIndices(algBuilder.toAlg(), algStartIdx, algEndIdx);
			if (this.tryConsumeNext(" ")) {
				...
				if (sq1PairStartMatch) {
					...
					const alg = addCharIndices(new Alg([addCharIndices(new Move(new QuantumMove("U_SQ_"), parseInt(topAmountString, 10)), savedCharIndex + 1, savedCharIndex + 1 + topAmountString.length), addCharIndices(new Move(new QuantumMove("D_SQ_"), parseInt(sq1PairEndMatch[1], 10)), savedCharIndexD, this.#idx - 1)]), savedCharIndex + 1, this.#idx - 1);
					algBuilder.push(addCharIndices(new Grouping(alg), savedCharIndex, this.#idx));
					crowded = true;
					algEndIdx = this.#idx;
				} else {
					const alg = this.parseAlgWithStopping([")"]);
					...
					algBuilder.push(addCharIndices(new Grouping(alg, amount), savedCharIndex, this.#idx));
					crowded = true;
					algEndIdx = this.#idx;
				}
			} else if (this.tryConsumeNext("^")) {
				if (!algDebugGlobals.caretNISSNotationEnabled) throw new Error("Alg contained a caret but caret NISS notation is not enabled.");
				this.mustConsumeNext("(");
				const alg = this.parseAlgWithStopping([")"]);
				this.popNext();
				...
				switch (separator) {
					case ":":
						unrepeated = addCharIndices(new Conjugate(A, B), savedCharIndex, this.#idx);
						crowded = true;
						algEndIdx = this.#idx;
						break;
					case ",":
						unrepeated = addCharIndices(new Commutator(A, B), savedCharIndex, this.#idx);
						crowded = true;
						algEndIdx = this.#idx;
						break;
					default: throw new Error("unexpected parsing error");
				}
				const afterClosingBracketIdx = this.#idx;
				const amount = this.parseAmount();
				if (amount === 1) algBuilder.push(unrepeated);
				else {
					const grouping = addCharIndices(new Grouping(addCharIndices(new Alg([unrepeated]), savedCharIndex, afterClosingBracketIdx), amount), savedCharIndex, this.#idx);
					algBuilder.push(grouping);
				}
				crowded = true;
				algEndIdx = this.#idx;
			} else if (this.tryConsumeNext("\n")) {
				algBuilder.push(addCharIndices(new Newline(), savedCharIndex, this.#idx));
				crowded = false;
				algEndIdx = this.#idx;
			} else if (this.tryConsumeNext("/")) if (this.tryConsumeNext("/")) {
				mustNotBeCrowded(savedCharIndex);
				const [text] = this.parseRegex(COMMENT_TEXT_REGEX);
				algBuilder.push(addCharIndices(new LineComment(text), savedCharIndex, this.#idx));
				crowded = false;
				algEndIdx = this.#idx;
			} else {
				algBuilder.push(addCharIndices(new Move("_SLASH_"), savedCharIndex, this.#idx));
				crowded = true;
				algEndIdx = this.#idx;
			}
			else if (this.tryConsumeNext(".")) {
	...
	parseMoveImpl() {
		const savedCharIndex = this.#idx;
		if (this.tryConsumeNext("/")) return addCharIndices(new Move("_SLASH_"), savedCharIndex, this.#idx);
		...
		if (suffix) {
			...
			if (suffix.startsWith("+")) quantumMove = quantumMove.modified({ family: `${quantumMove.family}_${suffix === "+" ? "PLUS" : "PLUSPLUS"}_` });
			if (suffix.startsWith("-")) {
				quantumMove = quantumMove.modified({ family: `${quantumMove.family}_${suffix === "-" ? "PLUS" : "PLUSPLUS"}_` });
				amount *= -1;
			}
		}
		return addCharIndices(new Move(quantumMove, amount), savedCharIndex, this.#idx);
	}
	parseMoveSuffix() {
		if (this.tryConsumeNext("+")) {
	...
	parseAmountAndTrackEmptyAbsAmount() {
		...
		return [parseIntWithEmptyFallback(absAmountStr, 1) * (primeStr === "'" ? -1 : 1), !absAmountStr];
	}
	parseAmount() {
		const savedIdx = this.#idx;
		const [, absAmountStr, primeStr] = this.parseRegex(AMOUNT_REGEX);
...
var QuantumWithAmount = class {
	...
	constructor(quantum, amount = 1) {
		...
		this.amount = amount;
		if (!Number.isInteger(this.amount) || this.amount < MIN_INT || this.amount > MAX_INT) throw new Error(`AlgNode amount absolute value must be a non-negative integer below ${MAX_INT_DESCRIPTION}.`);
	}
	suffix() {
		let s = "";
...
var QuantumMove = class _QuantumMove extends Comparable {
	...
	constructor(family, innerLayer, outerLayer) {
		...
		Object.freeze(this);
		if (this.#innerLayer !== null && (!Number.isInteger(this.#innerLayer) || this.#innerLayer < 1 || this.#innerLayer > MAX_INT)) throw new Error(`QuantumMove inner layer must be a positive integer below ${MAX_INT_DESCRIPTION}.`);
		if (this.#outerLayer !== null && (!Number.isInteger(this.#outerLayer) || this.#outerLayer < 1 || this.#outerLayer > MAX_INT)) throw new Error(`QuantumMove outer layer must be a positive integer below ${MAX_INT_DESCRIPTION}.`);
	...
	modified(modifications) {
		return new _QuantumMove(modifications.family ?? this.#family, modifications.innerLayer ?? this.#innerLayer, modifications.outerLayer ?? this.#outerLayer);
	}
	isIdentical(other) {
...
var Move = class _Move extends AlgCommon {
	...
	constructor(...args) {
		super();
		if (typeof args[0] === "string") if (args[1] ?? null) {
			this.#quantumWithAmount = new QuantumWithAmount(QuantumMove.fromString(args[0]), args[1]);
			return;
		} else return _Move.fromString(args[0]);
		this.#quantumWithAmount = new QuantumWithAmount(args[0], args[1]);
	}
	isIdentical(other) {
	...
	invert() {
		return transferCharIndex(this, new _Move(this.#quantumWithAmount.quantum, this.#isSlash() ? this.amount : -this.amount));
	}
	*experimentalExpand(iterDir = 1) {
		if (iterDir === 1) yield this;
		else yield this.modified({ amount: -this.amount });
	}
	get quantum() {
		return this.#quantumWithAmount.quantum;
	}
	modified(modifications) {
		return new _Move(this.#quantumWithAmount.quantum.modified(modifications), modifications.amount ?? this.amount);
	}
	static fromString(s) {
		return parseMove(s);
	}
	...
	toString(experimentalSerializationOptions) {
		...
		return this.#quantumWithAmount.quantum.toString(experimentalSerializationOptions) + this.#quantumWithAmount.suffix();
	}
};
var Square1TupleFormatter = class {
	...
	format(grouping, experimentalSerializationOptions) {
		if (experimentalSerializationOptions?.notation === "LGN") return null;
		if (grouping.amount !== 1) return null;
	...
	tuple(grouping) {
		...
		if (quantumAlg.experimentalNumChildAlgNodes() === 2) {
			...
			if (U.as(Move)?.quantum.isIdentical(this.quantumU_SQ_) && D.as(Move)?.quantum.isIdentical(this.quantumD_SQ_)) return [U, D];
		}
		return null;
	}
};
...
var Grouping = class _Grouping extends AlgCommon {
	...
	modified(modifications) {
		return new _Grouping(modifications.alg ?? this.alg, modifications.amount ?? this.amount);
	}
	/** @deprecated */
	...
	invert() {
		...
		return new _Grouping(this.#quantumWithAmount.quantum, -this.#quantumWithAmount.amount);
	}
	*experimentalExpand(iterDir = 1, depth) {
		depth ??= Infinity;
		if (depth === 0) yield iterDir === 1 ? this : this.invert();
	...
	#unrepeatedString(experimentalSerializationOptions) {
		const insideString = this.#quantumWithAmount.quantum.toString(experimentalSerializationOptions);
		const iter = this.alg.childAlgNodes();
		const { value } = iter.next();
		if (iter.next().done && (value?.is(Commutator) || value?.is(Conjugate))) return insideString;
		return `(${insideString})`;
	}
	toString(experimentalSerializationOptions) {
		return square1TupleFormatterInstance.format(this, experimentalSerializationOptions) ?? `${this.#unrepeatedString(experimentalSerializationOptions)}${this.#quantumWithAmount.suffix()}`;
	}
...
var AppendOptionsHelper = class {
	...
	cancelQuantum() {
		...
		if (cancel === true) return DEFAULT_DIRECTIONAL;
	...
	function modMove(move) {
		...
		switch (optionsHelper.cancelPuzzleSpecificModWrap()) {
			...
			case "preserve-sign":
				...
				break;
			default: throw new Error("Unknown mod wrap");
		}
	...
	if (optionsHelper.cancelAny()) {
		...
		for (i = outputPrefix.length - 1; i >= 0; i--) {
			...
			if (sameDirectionOnly) {
				...
				if (existingQuantumDirectionOnAxis && !areSameDirection(existingQuantumDirectionOnAxis, move)) break;
...
function experimentalAppendNode(alg, leaf, options) {
	...
	if (maybeMove) return experimentalAppendMove(alg, maybeMove, options);
	else return new Alg([...alg.childAlgNodes(), leaf]);
}
function dispatch(t, algNode, dataDown) {
	if (algNode.is(Grouping)) return t.traverseGrouping(algNode, dataDown);
	if (algNode.is(Move)) return t.traverseMove(algNode, dataDown);
	if (algNode.is(Commutator)) return t.traverseCommutator(algNode, dataDown);
	if (algNode.is(Conjugate)) return t.traverseConjugate(algNode, dataDown);
	if (algNode.is(Pause)) return t.traversePause(algNode, dataDown);
	if (algNode.is(Newline)) return t.traverseNewline(algNode, dataDown);
	if (algNode.is(LineComment)) return t.traverseLineComment(algNode, dataDown);
...
var Simplify = class extends TraversalDownUp {
	...
	*traverseAlg(alg, options) {
		...
		for (const algNode of alg.childAlgNodes()) for (const traversedNode of this.traverseAlgNode(algNode, newOptions)) output = Array.from(experimentalAppendNode(new Alg(output), traversedNode, newOptions).childAlgNodes());
	...
	*traverseGrouping(grouping, options) {
		...
		const newGrouping = new Grouping(this.traverseAlg(grouping.alg, this.#descendOptions(options)), grouping.amount);
	...
	*traverseMove(move, _options) {
	...
	#doChildrenCommute(A, B, options) {
		if (A.experimentalNumChildAlgNodes() === 1 && B.experimentalNumChildAlgNodes() === 1) {
			...
			if (new AppendOptionsHelper(options).puzzleSpecificSimplifyOptions()?.axis?.areQuantumMovesSameAxis(aMove.quantum, bMove.quantum)) return true;
		}
		return false;
	}
	*traverseCommutator(commutator, options) {
		...
		const newOptions = this.#descendOptions(options);
		const newCommutator = new Commutator(this.traverseAlg(commutator.A, newOptions), this.traverseAlg(commutator.B, newOptions));
	...
	*traverseConjugate(conjugate, options) {
		...
		const newOptions = this.#descendOptions(options);
		const newConjugate = new Conjugate(this.traverseAlg(conjugate.A, newOptions), this.traverseAlg(conjugate.B, newOptions));
...
function toIterable(input) {
	if (!input) return [];
	if (experimentalIs(input, Alg)) return input.childAlgNodes();
	if (typeof input === "string") return parseAlg(input).childAlgNodes();
	const iter = input;
...
var Alg = class _Alg extends AlgCommon {
	...
	constructor(alg) {
		...
		for (const algNode of this.#algNodes) if (!experimentalIsAlgNode(algNode)) throw new Error("An alg can only contain alg nodes.");
	}
	/**
	...
	*/
	isIdentical(other) {
		...
		for (let i = 0; i < l1.length; i++) if (!l1[i].isIdentical(l2[i])) return false;
		return true;
	}
	/**
	...
	*experimentalExpand(iterDir = 1, depth) {
		depth ??= Infinity;
		for (const algNode of direct(this.#algNodes, iterDir)) yield* algNode.experimentalExpand(iterDir, depth);
	}
	/**
	...
	*/
	expand(options) {
		return new _Alg(this.experimentalExpand(1, options?.depth ?? Infinity));
	}
	/** @deprecated */
	*experimentalLeafMoves() {
	...
	concat(input) {
		return new _Alg(Array.from(this.#algNodes).concat(Array.from(toIterable(input))));
	}
	/** @deprecated */
	experimentalIsEmpty() {
	...
	*/
	toString(experimentalSerializationOptions) {
		...
		for (const algNode of this.#algNodes) {
			if (previousVisibleAlgNode) output += spaceBetween(previousVisibleAlgNode, algNode);
			...
			} else if (algNode.as(Grouping)?.experimentalNISSPlaceholder) {} else output += algNode.toString(experimentalSerializationOptions);
...
function spaceBetween(u1, u2) {
	...
	if (u2.as(Grouping)?.experimentalNISSPlaceholder) return "";
...
new Alg([
	...
	new Move("U", -1),
	new Move("R", -1)
]), new Alg([new Commutator(new Alg([
	new Move("R", 1),
	new Move("U", 1),
	new Move("R", -2)
]), new Alg([new Conjugate(new Alg([new Move("R", 1)]), new Alg([new Move("U", 1)]))]))]), new Alg([
	new Move("R", 1),
	new Move("U", -1),
	new Move("L", -1),
	new Move("U", 1),
	...
	new Move("x", -1),
	new Commutator(new Alg([new Conjugate(new Alg([new Move("R", 1)]), new Alg([new Move("U", -1)]))]), new Alg([new Move("D", 1)])),
	new Commutator(new Alg([new Conjugate(new Alg([new Move("R", 1)]), new Alg([new Move("U", 1)]))]), new Alg([new Move("D", 1)])),
	new Move("x", 1)
]), new Alg([new Conjugate(new Alg([new Move("F", 1)]), new Alg([new Commutator(new Alg([new Move("U", 1)]), new Alg([new Move("R", 1)]))]))]), new Alg([new Conjugate(new Alg([new Move("R", 2)]), new Alg([new Commutator(new Alg([new Move("F", 2)]), new Alg([
	new Move("R", -1),
	new Move("B", -1),
	new Move("R", 1)
]))]))]), new Alg([
	...
	new Move("R", -1),
	new Move("F", -1)
]), new Alg([new Conjugate(new Alg([new Move("F", 1)]), new Alg([new Grouping(new Alg([new Commutator(new Alg([new Move("R", 1)]), new Alg([new Move("U", 1)]))]), 3)]))]), new Alg([
	new Pause(),
	new Pause(),
	new Pause()
]);
//#endregion
export { Grouping as a, QuantumMove as c, functionFromTraversal as d, Conjugate as i, TraversalDownUp as l, AlgBuilder as n, Move as o, Commutator as r, Pause as s, Alg as t, TraversalUp as u };
