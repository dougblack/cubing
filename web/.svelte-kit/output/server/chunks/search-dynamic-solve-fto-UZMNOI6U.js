...
import { n as randomUIntBelow } from "./esm.js";
//#region ../core/node_modules/cubing/dist/lib/cubing/chunks/search-dynamic-solve-fto-UZMNOI6U.js
var DEBUG = false;
var mathlib = (function() {
	var fact = [1];
	for (var i = 0; i < 32; ++i) fact[i + 1] = fact[i] * (i + 1);
	function circle(arr) {
		var length = arguments.length - 1, temp = arr[arguments[length]];
		for (var i2 = length; i2 > 1; i2--) arr[arguments[i2]] = arr[arguments[i2 - 1]];
	...
	function setNPerm(arr, idx, n, even) {
		...
		if (n >= 16) {
			...
			for (var i2 = n - 2; i2 >= 0; i2--) {
				...
				prt ^= arr[i2];
				idx = ~~(idx / (n - i2));
				for (var j = i2 + 1; j < n; j--) arr[j] >= arr[i2] && arr[j]++;
			}
			if (even < 0 && (prt & 1) != 0) {
				var tmp = arr[n - 1];
				arr[n - 1] = arr[n - 2];
	...
	function getNPerm(arr, n, even) {
		...
		if (n >= 16) {
			for (var i2 = 0; i2 < n - 1; i2++) {
				idx *= n - i2;
				for (var j = i2 + 1; j < n; j++) arr[j] < arr[i2] && idx++;
			}
	...
	function getNOri(arr, n, evenbase) {
		...
		for (var i2 = n - 1; i2 > 0; i2--) idx = idx * base + arr[i2] % base;
		return idx;
	}
	...
	function setMPerm(arr, idx, n, cnts, x) {
		for (var i2 = 0; i2 < n; i2++) for (var j = 0; j < cnts.length; j++) {
	...
	function Coord(type, length, evenbase) {
		...
		if (type == "p") {
			...
			this.cums = [0];
			for (var i2 = 1; i2 <= this.cntn; i2++) this.cums[i2] = this.cums[i2 - 1] + cnts[i2 - 1];
			this.n = this.cums[this.cntn];
			var n = this.n;
			var x = 1;
			for (var i2 = 0; i2 < this.cntn; i2++) for (var j = 1; j <= cnts[i2]; j++, n--) x *= n / j;
			this.x = Math.round(x);
			this.get = function(arr) {
		...
		for (var i2 = 0; i2 < facelets.length; i2++) {
			...
			outer_loop: for (var j = 0; j < facelets.length + 1; j++) {
				...
				for (var o = 0; o < n_ori; o++) {
					var isMatch = true;
					for (var t = 0; t < n_ori; t++) if (~~(facelets[j][t] / divcol) != f[facelets[i2][(t + o) % n_ori]]) {
						isMatch = false;
						break;
					}
					if (isMatch) {
	...
	function createMoveHash(initState, validMoves, hashFunc, moveFunc) {
		...
		for (var i2 = 0; i2 < states.length; i2++) {
			if (i2 == depthEnds[depthEnds.length - 1]) depthEnds.push(states.length);
			if (i2 % 1e4 == 9999) DEBUG && console.log(i2, "states scanned, tt=", +/* @__PURE__ */ new Date() - tt);
	...
	function createPrun(prun, init, size, maxd, doMove, N_MOVES, N_POWER, N_INV) {
		...
		N_INV = N_INV || 256;
		maxd = maxd || 256;
		for (var i2 = 0, len = size + 7 >>> 3; i2 < len; i2++) prun[i2] = -1;
		if (!Array.isArray(init)) init = [init];
		for (var i2 = 0; i2 < init.length; i2++) prun[init[i2] >> 3] ^= 15 << ((init[i2] & 7) << 2);
		var val = 0;
	...
	_.solveMulti = function(idxs, minl, MAXL, callback) {
		...
		outer_loop: for (var l = minl; l <= MAXL; l++) {
			for (var s = 0; s < idxs.length; s++) {
				this.sidx = s;
				if (this.idaSearch(idxs[s], l, -1, sol) == 0) break outer_loop;
			}
			this.sidx = -1;
		}
	...
	_.idaSearch = function(idx, maxl, lm, sol) {
		...
		if (prun == 0 && this.isSolved(idx) && maxl == 1) return 1;
	...
	function rndPerm(n, isEven) {
		var p = 0;
		var arr = [];
		for (var i2 = 0; i2 < n; i2++) arr[i2] = i2;
		for (var i2 = 0; i2 < n - 1; i2++) {
			var k = rn(n - i2);
			circle(arr, i2, i2 + k);
			p ^= k != 0;
	...
	function valuedArray(len, val) {
		...
		for (var i2 = 0; i2 < len; i2++) ret[i2] = isFun ? val(i2) : val;
		return ret;
	}
	function permOriMult(p1, p2, prod, o1, o2, ori, oriMod) {
		for (var i2 = 0; i2 < p2.length; i2++) {
			if (oriMod) ori[i2] = (o1[p2[i2]] + o2[i2]) % oriMod;
			prod[i2] = p1[p2[i2]];
		}
	}
	return {
...
var ftosolver = (function() {
	...
	FtoCubie.prototype.isEqual = function(fc) {
		for (var i2 = 0; i2 < 12; i2++) if (this.ep[i2] != fc.ep[i2] || this.uf[i2] != fc.uf[i2] || this.rl[i2] != fc.rl[i2] || i2 < 6 && (this.cp[i2] != fc.cp[i2] || this.co[i2] != fc.co[i2])) return false;
		return true;
	};
	FtoCubie.prototype.toFaceCube = function(todiv) {
		...
		todiv = todiv || 9;
		var co = [];
		for (var i2 = 0; i2 < 6; i2++) co[i2] = this.co[i2] * 2;
	...
	FtoCubie.prototype.fromFacelet = function(facelet) {
		...
		if (mathlib.getNParity(mathlib.getNPerm(this.uf, 12), 12) != 0) for (var i2 = 0; i2 < 12; i2++) this.uf[i2] ^= this.uf[i2] < 2 ? 1 : 0;
		if (mathlib.getNParity(mathlib.getNPerm(this.rl, 12), 12) != 0) for (var i2 = 0; i2 < 12; i2++) this.rl[i2] ^= this.rl[i2] < 2 ? 1 : 0;
		return this;
	};
	...
	function initMoveCube() {
		...
		var moveHash = [];
		for (var i2 = 0; i2 < 24; i2++) moveHash[i2] = moveCube[i2].ep.join(",");
		var symCube = [];
		var symMult = [];
		var symMulI = [];
		var symMulM = [];
		...
		for (var i2 = 0; i2 < 12; i2++) for (var j = 0; j < 12; j++) {
	...
	function ftoPermMove(key, perm, move) {
		...
		for (var i2 = 0; i2 < 12; i2++) ret[i2] = perm[movePerm[i2]];
		return ret;
	}
	...
	function phase1EdgeHash(ep) {
		...
		var e3fst = -1;
		for (var i2 = 0; i2 < 12; i2++) {
			if ((56 >> ep[i2] & 1) == 0) continue;
			if (e3fst == -1) e3fst = ep[i2];
	...
	function phase1CtrlHash(rl) {
		var ret = 0;
		for (var i2 = 0; i2 < 12; i2++) if (rl[i2] < 3) ret |= 1 << i2;
		return ret;
	}
	...
	function phase2CtHash(ct) {
		var ret = 0;
		for (var i2 = 0; i2 < 12; i2++) ret |= ~~(ct[i2] / 3) << i2 * 2;
		return ret;
	}
	...
	function randomMoves(validMoves, len) {
		var scramble = [];
		for (var i2 = 0; i2 < len; i2++) scramble.push(validMoves[~~(Math.random() * validMoves.length)]);
		var fc = new FtoCubie();
		for (var i2 = 0; i2 < scramble.length; i2++) fc = FtoCubie.FtoMult(fc, FtoCubie.moveCube[scramble[i2]], null);
		return [fc, scramble];
	}
	...
	function phase1ProcSol(sol, solsym, fc) {
		for (var i2 = 0; i2 < sol.length; i2++) sol[i2] = phase1Moves[sol[i2][0]] + sol[i2][1];
	...
	function solvePhase1(fc) {
		...
		for (var i2 = 0; i2 < p1sols.length; i2++) p1sols[i2].push(tt);
		return p1sols;
	}
	var phase2Moves = [
		0,
	...
	function phase2CpcoHash(fc) {
		...
		if (!(ret in p2cc2ufBit)) {
			var co = [];
			for (var i2 = 0; i2 < 6; i2++) co[i2] = fc.co[i2] * 2;
	...
	function phase2ufStd(uf, symMap) {
		...
		for (var i2 = 1; i2 < 12; i2++) if (uf[i2] != col1) {
			col2 = uf[i2];
			break;
		}
		var sym = symMap[col1 * 4 + col2];
		for (var i2 = 0; i2 < 12; i2++) uf[i2] = ~~(FtoCubie.symCube[sym].uf[uf[i2] * 3] / 3);
		return sym;
	}
	function getPhase2ufIdx(uf) {
		var ufstd = [];
		for (var i2 = 0; i2 < 12; i2++) ufstd[i2] = ~~(uf[i2] / 3);
	...
	function phase2Init() {
		...
		solv2 = new mathlib.Searcher(function(idx2) {
			...
			return Math.max(Math.min(P2EPRL_MAXL, mathlib.getPruning(p2eprlPrun, idx2[1] * N_P2EP + idx2[0])), p2necPrun[necIdx]);
		}, function(idx2, move) {
	...
	function solvePhase2(solvInfos) {
		...
		var tt = performance.now();
		var idxs = [];
		for (var i2 = 0; i2 < solvInfos.length; i2++) idxs.push([
	...
	function applyMoves(fc, moves) {
		for (var i2 = 0; i2 < moves.length; i2++) fc = FtoCubie.FtoMult(fc, FtoCubie.moveCube[moves[i2]], null);
		return fc;
	}
	var move2str = [
		"U",
	...
	function prettyMoves(moves) {
		var buf = [];
		for (var i2 = 0; i2 < moves.length; i2++) buf[i2] = move2str[moves[i2]];
	...
	FtoSolver.prototype.solveFto = function(fc, invSol) {
		...
		if (invSol) {
			for (var i2 = 0; i2 < sol.length; i2++) sol[i2] ^= 1;
			sol.reverse();
		}
	...
	function solveTest(n_moves) {
		...
		var isSolved = true;
		outer_loop: for (var i2 = 0; i2 < 8; i2++) for (var j = 1; j < 9; j++) if (facelets[i2 * 9 + j] != facelets[i2 * 9]) {
			isSolved = false;
			break outer_loop;
		}
	...
	function testbench(ntest) {
		...
		for (var nsolv = 0; nsolv < ntest; nsolv++) {
			...
			for (var i2 = 0; i2 < lengths.length; i2++) cumlen[i2] = (cumlen[i2] || 0) + lengths[i2];
...
var fto_scrambler = (function() {
	...
	return { getRandomScramble: getRandomScramble.bind(null, false, false, false) };
})();
function getRandomFTOScramble() {
	return fto_scrambler.getRandomScramble();
}
//#endregion
export { getRandomFTOScramble };
