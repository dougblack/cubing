import { n as AlgBuilder, o as Move, t as Alg } from "./chunk-O6HEZXGY.js";
import { i as KPattern, n as puzzles, r as from } from "./chunk-FLK6AZKB.js";
import { t as randomChoice } from "./esm.js";
//#region ../core/node_modules/cubing/dist/lib/cubing/chunks/chunk-V27EM5TJ.js
...
function addOrientationSuffix(alg, suffixSpec) {
	...
	for (const suffix of suffixSpec) {
		...
		if (choice !== null) algBuilder.push(Move.fromString(choice));
	}
...
function toReid333Struct(pattern) {
	const output = [[], []];
	for (let i = 0; i < 6; i++) if (pattern.patternData["CENTERS"].pieces[i] !== i) throw new Error("non-oriented puzzles are not supported");
	for (let i = 0; i < 12; i++) output[0].push(rotateLeft(reidEdgeOrder[pattern.patternData["EDGES"].pieces[i]], pattern.patternData["EDGES"].orientation[i]));
	for (let i = 0; i < 8; i++) output[1].push(rotateLeft(reidCornerOrder[pattern.patternData["CORNERS"].pieces[i]], pattern.patternData["CORNERS"].orientation[i]));
...
function passesFilter(kpuzzle, pattern) {
	if (isEquivalentTranformationIgnoringCENTERS(kpuzzle.defaultPattern(), pattern)) return false;
	for (const face of "ULFRBD") for (let amount = 1; amount < 4; amount++) if (isEquivalentTranformationIgnoringCENTERS(kpuzzle.moveToTransformation(new Move(face, amount)).toKPattern(), pattern)) return false;
	return true;
}
var sgs3x3x3 = [
	[
...
async function random333Pattern() {
	...
	for (const piece of sgs3x3x3) pattern = pattern.applyAlg(Alg.fromString(randomChoice(piece)));
	if (!passesFilter(kpuzzle, pattern)) return random333Pattern();
	return pattern;
}
async function solve333(s) {
	mustBeInsideWorker();
	return Alg.fromString((await dynamic3x3x3min2phase).solvePattern(toMin2PhasePattern(s)));
}
...
function nodeEndpoint(rawEndpoint) {
	...
	return {
		...
		addEventListener: (event, handler, options) => {
			const listener = (data) => {
				if (options?.once) removeEventListener(event, listener);
				if ("handleEvent" in handler) handler.handleEvent({ data });
				else handler({ data });
			};
		...
		terminate: rawEndpoint.terminate?.bind(rawEndpoint),
		close: rawEndpoint.close?.bind(rawEndpoint)
	};
}
function toEndpoint(endpoint) {
	if (!("addEventListener" in endpoint) || !("removeEventListener" in endpoint)) return nodeEndpoint(endpoint);
	return endpoint;
}
...
function wrap(endpoint, target, options) {
	...
	ep.addEventListener("message", function handleMessage(ev) {
		...
		try {
			...
			if (pendingListeners.size === 0 && (options?.refCount ?? REF_COUNT_BY_DEFAULT)) ep.unref?.();
		}
	});
	return createProxy({
		endpoint: ep,
...
function releaseEndpoint(epWithPendingListeners) {
	return requestResponseMessage(epWithPendingListeners, { type: "RELEASE" }).then(() => {
...
var proxyFinalizers = "FinalizationRegistry" in globalThis && new FinalizationRegistry((epWithPendingListeners) => {
	...
	if (newCount === 0) releaseEndpoint(epWithPendingListeners).finally(() => {
...
function registerProxy(proxy2, epWithPendingListeners) {
	const newCount = (proxyCounter.get(epWithPendingListeners) || 0) + 1;
	proxyCounter.set(epWithPendingListeners, newCount);
	if (proxyFinalizers) proxyFinalizers.register(proxy2, epWithPendingListeners, proxy2);
}
...
function createProxy(epWithPendingListeners, path = [], target = () => {}) {
	...
	const proxy2 = new Proxy(target, {
		...
		apply(_target, _thisArg, rawArgumentList) {
			...
			if (last === createEndpoint) return requestResponseMessage(epWithPendingListeners, { type: "ENDPOINT" }).then(fromWireValue);
			if (last === "bind") return createProxy(epWithPendingListeners, path.slice(0, -1));
...
function isCrossOrigin(url) {
	...
	return globalThis.location.origin !== new URL(url, globalThis.location.href).origin;
}
function constructPortableWebWorker(url, workerOptions) {
	...
	if (useTrampoline) {
		...
		const blob = new Blob([trampolineSource], { type: "text/javascript" });
	...
	if (useTrampoline) {
		...
		Object.defineProperty(worker, "terminate", { get() {
...
function constructNodeStyleWorker(url, workerOptions) {
	const { Worker: NodeWorker } = globalThis.process.getBuiltinModule("node:worker_threads");
...
function constructPortableWorker(url, workerOptions) {
	...
	if (hasWebWorkers && !hasBuiltinModules) return constructPortableWebWorker(url, workerOptions);
	const webWorkersHaveUnref = globalThis.Worker?.prototype.unref;
	if (hasWebWorkers && hasBuiltinModules && webWorkersHaveUnref) return constructPortableWebWorker(url, workerOptions);
	else return constructNodeStyleWorker(url, workerOptions);
}
...
*/
//#endregion
export { mustBeInsideWorker as a, setIsInsideWorker as c, initialize333 as i, solve333 as l, addOrientationSuffix as n, random333OrientedScramble as o, expose as r, random333Scramble as s, PortableWorker as t, wrap as u };
