//#region ../core/node_modules/cubing/dist/lib/cubing/chunks/search-dynamic-solve-3x3x3-B2L4IN34.js
var $intern_3 = { 3: 1 };
...
function defineClass(typeId, superTypeId, castableTypeMap) {
	...
	for (let i = 3; i < arguments.length; ++i) arguments[i].prototype = _;
	clazz && (_.___clazz$ = clazz);
}
function createSubclassPrototype_0(superTypeId) {
	return portableObjCreate(prototypesByTypeId_0[superTypeId]);
}
function Object_0() {}
...
function createForClass(packageName, compoundClassName, typeId) {
	var clazz = createClassObject(packageName, compoundClassName);
...
function createForPrimitive(className, primitiveTypeId) {
	var clazz = createClassObject("", className);
...
function getPrototypeForClass(clazz) {
	...
	return prototypesByTypeId_0[clazz.typeId];
}
...
function initDim(leafClassLiteral, castableTypeMap, elementTypeId, length_0, elementTypeCategory, dimensions) {
	var result = initializeArrayElementsWithDefaults(elementTypeCategory, length_0);
...
function fromInt(value_0) {
	...
	if (value_0 > -129 && value_0 < 128) {
		rebase = value_0 + 128;
		boxedValues ??= initDim(Lcom_google_gwt_lang_LongLibBase$LongEmul_2_classLit, $intern_3, 293, 256, 0, 1);
...
function equals_7(array1, array2) {
	...
	for (i = 0; i < array1.length; ++i) if (array1[i] !== array2[i]) return false;
	return true;
}
...
function CornConjugate(a, idx, b) {
	...
	var oriB;
	var s;
	var sinv = CubeSym[SymMultInv[0][idx]];
	s = CubeSym[idx];
		...
		for (s = 0; s < 16; s += sym_inc) {
			...
			switch (coord) {
				...
				case 1:
					idx = $getTwist(d);
					break;
				case 2: idx = getNPerm(d.ea, 8, true);
			}
...
function $phase1(this$static, node, ssym, maxl, lm) {
	...
	var ret;
	var skipMoves;
	if (node.prun === 0 && maxl < 5) if (this$static.allowShorter || maxl === 0) {
...
function $solution(this$static, facelets) {
	var check = $verify_0(this$static, facelets);
...
function toCubieCube(f, ccRet) {
	...
	for (i2 = 0; i2 < 8; i2++) {
		for (ori = 0; ori < 3; ori++) if (f[cornerFacelet[i2][ori]] === 0 || f[cornerFacelet[i2][ori]] === 3) break;
...
function $toString_2(this$static) {
	...
	if (urf < 3) for (s = 0; s < this$static.length_0; s++) {
		this$static.verbose & 1 && s === this$static.depth1 && (sb.string += ".  ");
...
//#endregion
export { initialize, solvePattern };
