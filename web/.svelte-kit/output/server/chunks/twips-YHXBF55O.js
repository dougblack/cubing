...
//#region ../core/node_modules/cubing/dist/lib/cubing/chunks/twips-YHXBF55O.js
var wasm;
...
function wasmFreeMemoryForAllScrambleFinders() {
	return wasm.wasmFreeMemoryForAllScrambleFinders() >>> 0;
}
...
async function __wbg_init(module_or_path) {
	...
	if (typeof module_or_path === "undefined") throw new Error("Only base 64 WASM loading is supported at the moment.");
...
async function wasmDeriveScrambleForEvent2(hexDerivationSeed, derivationSaltHierarchy, eventId) {
	for (const derivationSalt of derivationSaltHierarchy) if (derivationSalt.includes("/")) throw new Error("Derivation salts cannot contain slashes.");
	await initWrapper();
	return new Alg(wasmDeriveScrambleForEvent(hexDerivationSeed, derivationSaltHierarchy.join("/"), eventId));
}
async function wasmTwips2(kpuzzleDefinition, searchPattern, options) {
	await initWrapper();
	return new Alg(wasmTwips(JSON.stringify(kpuzzleDefinition), JSON.stringify(searchPattern.toJSON()["patternData"]), JSON.stringify(options)));
}
function wasmFreeMemoryForAllScrambleFinders2() {
	return wasmFreeMemoryForAllScrambleFinders();
}
//#endregion
export { wasmDeriveScrambleForEvent2 as wasmDeriveScrambleForEvent, wasmFreeMemoryForAllScrambleFinders2 as wasmFreeMemoryForAllScrambleFinders, wasmRandomScrambleForEvent2 as wasmRandomScrambleForEvent, wasmTwips2 as wasmTwips };
