...
//#region ../core/node_modules/cubing/dist/lib/cubing/chunks/chunk-ROB5TROI.js
function parseSGS(kpuzzle, sgs) {
	const subgroupSizes = [];
	const sgsActions = [];
	for (const line of sgs.split("\n")) {
		const lineTokens = line.split(" ");
		if (line.startsWith("SetOrder ")) {} else if (line.startsWith("Alg ")) {
			const alg = Alg.fromString(line.substring(4));
			sgsActions.push({
				alg,
				transformation: kpuzzle.algToTransformation(alg)
			});
		} else if (line.startsWith("SubgroupSizes ")) for (let j = 1; j < lineTokens.length; j++) subgroupSizes.push(parseInt(lineTokens[j], 10));
	}
	const sgsCachedData = { ordering: new Array(subgroupSizes.length) };
	...
	for (const orbitDefinition of kpuzzle.definition.orbits) processedPieces[orbitDefinition.orbitName] = new Array(orbitDefinition.numPieces).fill(false);
	for (let i = subgroupSizes.length - 1; i >= 0; i--) {
		...
		for (let j = subgroupAlgOffsets[i]; j < subgroupAlgOffsets[i + 1]; j++) {
			...
			for (const orbitDefinition of kpuzzle.definition.orbits) for (let idx = 0; idx < orbitDefinition.numPieces; idx++) if (transformation.transformationData[orbitDefinition.orbitName].permutation[idx] !== idx || transformation.transformationData[orbitDefinition.orbitName].orientationDelta[idx] !== 0) {
